...
#   Maintenance > Show startup log / Detailed logs / Text consoles.
...
case "${1:-}" in
    "") : ;;
    debug|release)
        echo "note: '$1' is no longer a build mode -- one ISO for all; the debug features are toggles in LayerOSX Settings (Maintenance > Detailed logs, Text consoles)." ;;
