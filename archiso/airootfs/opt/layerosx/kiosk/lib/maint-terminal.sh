#!/usr/bin/env bash
# F2 maintenance terminal. Policy is baked at build time into
...
#   off      -> F2 does nothing (fully locked appliance).
#
...
# to ~/maint-auth.log with a timestamp only. 3 tries per F2 press, with a short
...
# One terminal at a time (F2 mashed repeatedly shouldn't stack dialogs).
