...
# forced on everyone all the time.
#
...
# zenity dialogs, the QEMU/SDL window, ...) is lost -- this only adds
# one keybinding on top of it, idempotently (safe to call every boot).
set -uo pipefail
...
grep -q 'layerosx-f2-peek' ~/.config/openbox/rc.xml 2>/dev/null && exit 0
