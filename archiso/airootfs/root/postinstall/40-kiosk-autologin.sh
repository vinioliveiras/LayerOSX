...
# Monitoring mode, if it was left on (Settings > Maintenance / macmonitor on).
[ "$(cat /var/lib/layerosx/monitoring 2>/dev/null)" = on ] && /usr/local/bin/macmonitor start >/dev/null 2>&1
